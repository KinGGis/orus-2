-- Revert: restrict quote_mode back to MARKET | MANUAL.
-- Any rows with INTERNAL_YTM are switched to MANUAL to avoid CHECK violation.

PRAGMA foreign_keys = OFF;

UPDATE assets SET quote_mode = 'MANUAL' WHERE quote_mode = 'INTERNAL_YTM';

ALTER TABLE assets RENAME TO assets_old_post_ytm;
...
SELECT
    ...
    instrument_type, instrument_symbol, instrument_exchange_mic,
    provider_config, created_at, updated_at
FROM assets_old_post_ytm;

DROP TABLE assets_old_post_ytm;
